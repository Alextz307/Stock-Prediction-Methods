...
class BacktestEngine:
    ...
    def _process_ticker(self, ticker: str, start_date: datetime, end_date: datetime):
        print(f"\nProcessing {ticker}...")
    ...
    def _record_result(self, ticker: str, strat_name: str, returns: pd.Series):
        metrics = calculate_metrics(returns)
        metrics["Ticker"] = ticker
        metrics["Strategy"] = strat_name
        self.all_results.append(metrics)
        
        print(f"    {strat_name} | Sharpe: {metrics.get('Sharpe Ratio', 0):.2f} | Return: {metrics.get('Total Return', 0)*100:.2f}%")
    ...
    def _finalize(self):
        ...
        cols = ["Ticker", "Strategy", "Sharpe Ratio", "Total Return", "Max Drawdown", "Win Rate"]
