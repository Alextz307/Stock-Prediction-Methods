...
class VolatilityTargetingStrategy(BaseStrategy):
    ...
    def __init__(self, target_vol: float = 0.15, trend_window: int = 200, ma_type: str = "sma", hybrid_model: HybridModel | None = None):
        """
        Args:
    ...
    def train(self, df: pd.DataFrame) -> None:
        ...
        """
        self.model.train(df, target_col="vol_garman_klass")
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.Series:
        ...
        try:
            ...
            if self.trend_window > 0:
                ...
                signals = signals * is_bullish.astype(int)
...
class AdaptiveBollingerStrategy(BaseStrategy):
    ...
    def __init__(self, window: int = 20, k: float = 2.0, trend_window: int = 200, ma_type: str = "sma"):
        """
        Args:
...
class MomentumGatekeeperStrategy(BaseStrategy):
    ...
    def __init__(self, ma_window: int = 50, prob_threshold: float = 0.55):
        """
        Args:
        ...
        self.classifier = DirectionalClassifier()
        self.fitted = False
    ...
    def _get_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'rsi_14' not in df.columns:
            delta = df['close'].diff()
            gain = (delta.where(delta > 0, 0)).rolling(settings.RSI_PERIOD).mean()
            loss = (-delta.where(delta < 0, 0)).rolling(settings.RSI_PERIOD).mean()
            rs = gain / loss
            df['rsi_14'] = 100 - (100 / (1 + rs))
            
        features = ['return_1d', 'vol_20', 'ma_ratio', 'rsi_14', 'volume']
